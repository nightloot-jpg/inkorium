...
## 2026-07-21 - Visual Consistency in Sidebar Layouts

**Learning:** When aligning the styling of multiple UI sections across columns in a complex layout (like a Tuenti-style feed), small inconsistencies in padding (e.g., `p-3` vs `p-5`), border styles, border radiuses (e.g., `rounded-[10px]` vs `rounded-sm`), and typography for headings and interactive links heavily disrupt visual rhythm. Enforcing a single design system using utility classes makes a huge difference. Also, relying directly on visual testing via tools like Playwright and capturing screenshots allows to verify CSS utility class changes properly in the context of the running application.
...
## 2024-05-18 - Supabase Type Generation Pitfall

**Learning:** When generating Typescript types for Supabase using the CLI (`supabase gen types typescript`), it overwrites the entire file. If there were manual modifications (like extending the Enum for new tables or tweaking properties for types from other schema), they might be lost, causing type errors in unrelated files. It's safer to generate types in a temporary file and merge, or to trust the generation only if the database schema is complete and fully reflective of all necessary enums. In this case, `post_type` enum was missing the `'event'` value and `friendships` missing, possibly due to it running against an older schema version if not fully synced, or just the nature of `supabase gen types`. I had to manually append the missing tables and enums.
**Action:** When asked to update Supabase types, always perform a diff after generation. If unrelated tables are dropped or enums are lost, manually inject the newly created tables into the previous `types.ts` instead of accepting the overwritten file completely.

## 2024-05-18 - Replacing Mocks with Real Data using TanStack Query

**Learning:** When refactoring a component from static mocks to a database backend using `useQuery`, type safety across boundaries is crucial. Specifically, returning joined data from Supabase (e.g. `event_attendees(count)`) alters the expected row type from just `Database["public"]["Tables"]["events"]["Row"]`. Instead of casting using `as any` or `as unknown`, define an explicit interface like `interface EventWithCounts extends EventRow { event_attendees: { count: number }[] }`.
**Action:** When mapping database return data in a component prop, explicitly type the expected shape of joins. Don't rely on `as any` because subsequent type checks and linting will fail. Ensure components gracefully handle `isLoading` and `error` states which mock data inherently lacked.

## 2026-07-23 - Complex Filters User Journey\n\n**Learning:** Implementing complex frontend filtering and sorting efficiently required merging search parameters directly to URL state while avoiding infinite loops. Additionally, Playwright locator strategy requires explicit awaits and using robust selection (like `waitForSelector` and `evaluate`) when testing complex UI such as conditionally rendered buttons. Testing inputs with Playwright needs full `clear()` or a precise control sequence to wipe fields between strokes without causing elements to be marked detached.\n\n**Action:** Use `searchParams` from the router to power `useMemo` hooks and bind URL logic cleanly to React state without relying purely on local disjoint state. Always use precise Playwright waits, input clearing methods, and evaluate clicks for complex DOM changes.

## 2026-07-23 - Event Cards Modernization
...
## 2026-07-23 - Eventos e2e conectados

**Learning:** Generar e insertar payloads complejos a base de datos suele dar problemas con los tipos autogenerados de Supabase si la estructura diverge ligeramente (por ejemplo, omitiendo campos opcionales o añadiendo extras al insertar). La interfaz genérica de TypeScript requiere que cada clave esté correctamente tipada, lo que en objetos formados con desestructuración puede saltar fácilmente por propiedades perdidas como .
**Action:** En flujos complejos que actualicen múltiples campos o permitan edición y creación a la vez, mapear explícitamente el `Insert` y `Update` payload por separado y, cuando estemos bloqueados, utilizar aserciones tipo `as any` o actualizar los `types.ts` y reiniciar el servidor de Typescript para asegurar que pasa localmente y en el CI de commit, evitando retrasos productivos.

## 2026-07-23 - Eventos e2e conectados

**Learning:** Generar e insertar payloads complejos a base de datos suele dar problemas con los tipos autogenerados de Supabase si la estructura diverge ligeramente (por ejemplo, omitiendo campos opcionales o añadiendo extras al insertar). La interfaz genérica de TypeScript events requiere que cada clave esté correctamente tipada, lo que en objetos formados con desestructuración puede saltar fácilmente por propiedades perdidas.
**Action:** En flujos complejos que actualicen múltiples campos o permitan edición y creación a la vez, mapear explícitamente el Insert y Update payload por separado y utilizar aserciones o regenerar tipos evitando retrasos en Typescript.
