...
## 2024-05-24 - [Optimization React.memo for PostCard]
**Learning:** Found that `PostCard` in `src/components/post-card.tsx` was re-rendering unnecessarily during typing because of feed updates, missing a simple memoization layer.
**Action:** Used `React.memo()` on `PostCard` to prevent re-renders when parent states change, improving overall performance without sacrificing readability.
