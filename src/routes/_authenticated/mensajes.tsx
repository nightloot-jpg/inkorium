...
function MessagesLayout() {
  ...
  const { data: threads = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("messages")
        .select("id, content, created_at, read_at, sender_id, recipient_id, sender:profiles!messages_sender_id_fkey(id, username, display_name, avatar_url), recipient:profiles!messages_recipient_id_fkey(id, username, display_name, avatar_url)")
        .or(`sender_id.eq.${userId},recipient_id.eq.${userId}`)
        .order("created_at", { ascending: false })
        .limit(200);

      const seen = new Map<string, { other: { id: string; username: string; display_name: string; avatar_url: string | null }; last: string; created_at: string; unread: boolean }>();
      (data ?? []).forEach((m) => {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-6">
      <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-4 bg-card ring-1 ring-border rounded-2xl shadow-card overflow-hidden min-h-[70vh]">
        <aside className={`border-r border-border ${showListOnMobile ? "block" : "hidden md:block"}`}>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className={`text-xs truncate ${t.unread ? "text-foreground" : "text-muted-foreground"}`}>
                      {t.last}
                    </p>
                  </div>
                </Link>
